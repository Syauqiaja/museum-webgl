...
|---|---|---|
| `Museum.Core.Tests` | EditMode | `VideoUrlRules` — normalise (spaces only, idempotent), key sanitising, https/absolute validation; `PlayerAvatars` — the id list the server also holds, sanitising, name matching; `MuseumInteractions` — remote dispatch by station id, no echo of a remote play, the ids the server holds; `MuseumActivities.Tag`; `SceneMusic` — each scene's track, the Lobby playing its game's, `LobbyRequest.LastGameScene` outliving the cleared request |
| `Museum.Core.Tests.PlayMode` | PlayMode | `SessionData` — PlayerPrefs round-trip, sanitising on set, seat set/clear, minted `playerId`, `PlayerAvatar` (Jawa default, fallback, survives a session); `MainMenu` avatar row (Jawa selected by default, a pick stores and moves the frame, a returning pick is restored); the museum return pose (taken once, forgotten from the menu, not restored on a fresh session); shared stations (another visitor's engklek step lights the petak but leaves this visitor's run and status, their tembang leaves the lyric, only local presses are reported, linear rolloff); `GameAudio` (every button gets its sounds unwired, a track switch and the same track carrying on, the music dipping under a video and back, a destroyed screen not holding it down, the win dip, a `SilentButton` making no click) — with silent generated clips, no asset |
...
| `Museum.Games.Egrang.Tests.PlayMode` | PlayMode | `EgrangRace`, `EgrangRacer`, `EgrangStepMover` snapping, `SkillCheckBar.Configure`; `EgrangRacerBody` — a worn character stands over its walker, at the start line and after walking (uses the `Char_*.fbx` models, editor only); `SkillCheckBar` marking the buttons wired to its `Press` (JALAN) silent and no others |
