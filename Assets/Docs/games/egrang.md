...
The `KELUAR` button in the top-right of `Egrang UI` (generated by
`Museum/Egrang/Build Exit Button`, a sibling of the panels so it is reachable during the stilt
pick too) calls `EgrangRace.BackToMuseum` — a frozen UnityEvent handler name. That calls
`IEgrangSession.Leave()`, which `NetEgrangSession` routes through
`ColyseusNetManager.Leave(room)` so the leave is **consented** and the held reconnection token
is cleared, then unsubscribes, clears `SessionData`'s room session and loads the Museum. On
the server side a consented leave is the "racer leaves mid-race" row above: the racer is
withdrawn at once and the survivors finish normally. Leaving via the browser's back button
instead keeps the seat through the reconnection window, which is why the button exists.
